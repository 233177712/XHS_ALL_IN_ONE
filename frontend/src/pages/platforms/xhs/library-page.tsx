...
function formatSavedTime(v: string): string {
  return formatShanghaiTime(v);
}
function getRawNoteType(note: SavedNote): string {
  const t = note.raw_json?.model_type ?? note.raw_json?.type;
  return typeof t === "string" ? t : "note";
}
...
export function XhsLibraryPage() {
  ...
  function toggleNoteSelection(id: number) {
    setSelectedNoteIds((c) => (c.includes(id) ? c.filter((i) => i !== id) : [...c, id]));
  }
  function toggleVisibleSelection() {
    if (!notes.length) return;
    const vis = notes.map((n) => n.id);
    const allSel = vis.every((id) => selectedNoteIdSet.has(id));
    setSelectedNoteIds((c) => (allSel ? c.filter((id) => !vis.includes(id)) : Array.from(new Set([...c, ...vis]))));
  }
  ...
  async function openDetail(note: SavedNote) {
    ...
    try {
      const [d, a] = await Promise.all([fetchSavedNote(note.id), fetchSavedNoteAssets(note.id)]);
      setSelectedNote(d);
      setSelectedAssets(a.items);
    } catch {
      setDetailError("笔记详情加载失败。");
    } finally {
  ...
  async function copySelectedNote() {
    ...
    try {
      ...
      setDetailActionMessage("已复制标题和正文。");
    } catch {
      setDetailActionMessage("复制失败。");
    }
  }
  ...
  async function addToDrafts() {
    ...
    try {
      ...
      setDetailActionMessage("加入草稿工坊失败。");
    } finally {
      setIsCreatingDraft(false);
    }
  }
  ...
  async function createBatchRewriteDrafts() {
    if (!selectedNoteIds.length) {
      setBatchActionMessage("请先选择笔记。");
      return;
    }
    setIsBatchWorking(true);
    setBatchActionMessage(null);
    try {
      const r = await batchCreateDraftsFromNotes({ note_ids: selectedNoteIds, intent: "rewrite" });
      setBatchActionMessage(`已创建 ${r.created_count} 个改写草稿。`);
    } catch {
      setBatchActionMessage("批量创建草稿失败。");
    } finally {
  ...
  async function exportSelectedNotes(format: "json" | "csv") {
    ...
    setBatchActionMessage(null);
    try {
      const r = await exportSavedNotes({ note_ids: selectedNoteIds, format });
      setLatestExport(r);
      setBatchActionMessage(`已导出 ${r.exported_count} 条笔记。`);
    } catch {
      setBatchActionMessage("导出失败。");
    } finally {
      setIsBatchWorking(false);
    }
  }
  ...
  async function downloadLatestExport() {
    ...
    setBatchActionMessage(null);
    try {
      await downloadExportFile(latestExport.download_url, latestExport.file_name);
      setBatchActionMessage(`已下载：${latestExport.file_name}`);
    } catch {
      setBatchActionMessage("下载失败。");
    } finally {
      setIsBatchWorking(false);
    }
  }
  ...
  async function loadComments(page = 1) {
    ...
    setCommentsError(null);
    try {
      const r = await fetchSavedNoteComments(selectedNote.id, page);
      setComments((c) => (page === 1 ? r.items : [...c, ...r.items]));
      setCommentsTotal(r.total);
  ...
  function toggleComments() {
    ...
    if (next && selectedNote && comments.length === 0) void loadComments(1);
  }
  const topLevelComments = comments.filter((c) => !c.parent_comment_id);
  function childComments(pid: string) {
    return comments.filter((c) => c.parent_comment_id === pid);
  }
  ...
  const tableColumns: ColumnsType<SavedNote> = [
    {
      ...
      dataIndex: "title",
      ellipsis: true,
      render: (t: string, n) => <a onClick={() => void openDetail(n)}>{t || "未命名"}</a>,
    },
    ...
    <div>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
          ...
          <Text type="secondary">保存的笔记素材，支持标签、筛选、批量操作和导出</Text>
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={() => void loadNotes()} loading={isLoading}>
            刷新
          </Button>
        </Col>
      </Row>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card size="small">
            <Statistic title="已保存笔记" value={total} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic title="当前视图" value={viewMode === "card" ? "卡片" : "表格"} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic title="已选择" value={selectedNoteIds.length} suffix="条" />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
            <Statistic title="平台" value="XHS" />
          </Card>
        </Col>
      </Row>
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <Row gutter={12} align="middle">
          <Col span={5}>
            <Input placeholder="标题、正文、作者" value={keywordFilter} onChange={(e) => setKeywordFilter(e.target.value)} allowClear />
          </Col>
          <Col span={4}>
            <Select
              value={selectedTagFilter || undefined}
              onChange={(v) => setSelectedTagFilter(v ?? "")}
              placeholder="全部标签"
              allowClear
              style={{ width: "100%" }}
              options={availableTags.map((t) => ({ value: String(t.id), label: t.name }))}
            />
          </Col>
          <Col>
            <Checkbox checked={hasAssetsFilter} onChange={(e) => setHasAssetsFilter(e.target.checked)}>
              有素材
            </Checkbox>
          </Col>
          <Col>
            <Checkbox checked={hasCommentsFilter} onChange={(e) => setHasCommentsFilter(e.target.checked)}>
              有评论
            </Checkbox>
          </Col>
          <Col>
          ...
          <Col>
            <Button type="primary" onClick={() => void loadNotes()} loading={isLoading}>
              筛选
            </Button>
          </Col>
        </Row>
      ...
      {notes.length > 0 && (
        <Card size="small" style={{ marginBottom: 16 }}>
          <Space wrap>
            <Checkbox checked={notes.length > 0 && notes.every((n) => selectedNoteIdSet.has(n.id))} onChange={toggleVisibleSelection}>
              选择当前页
            </Checkbox>
            <Text strong>{selectedNoteIds.length} 条已选</Text>
            <Button
              icon={<CheckSquareOutlined />}
              disabled={isBatchWorking || !selectedNoteIds.length}
              onClick={createBatchRewriteDrafts}
              size="small"
            >
              批量加入草稿工坊
            </Button>
            ...
            <Button
              icon={<DownloadOutlined />}
              disabled={isBatchWorking || !selectedNoteIds.length}
              onClick={() => exportSelectedNotes("csv")}
              size="small"
            >
              CSV
            </Button>
            {latestExport && (
              <Button icon={<DownloadOutlined />} disabled={isBatchWorking} onClick={downloadLatestExport} size="small">
                下载
              </Button>
            )}
            <Popconfirm title={`确定删除选中的 ${selectedNoteIds.length} 条笔记？`} onConfirm={batchDeleteNotes}>
              <Button danger icon={<DeleteOutlined />} disabled={isBatchWorking || !selectedNoteIds.length} size="small">
                批量删除
              </Button>
            </Popconfirm>
            <Button disabled={!selectedNoteIds.length} onClick={clearSelection} size="small">
              清空选择
            </Button>
          </Space>
          {batchActionMessage && (
            <Alert
              message={batchActionMessage}
              type="info"
              showIcon
              style={{ marginTop: 8 }}
              closable
              onClose={() => setBatchActionMessage(null)}
            />
          )}
        </Card>
      )}
                  ...
                  cover={
                    <div style={{ position: "relative", background: "#262626" }}>
                      ...
                      <Tag
                        ...
                        icon={kind.includes("video") ? <PlayCircleOutlined /> : <PictureOutlined />}
                      >
                ...
                >
                    ...
                    description={
                      <>
                        <div>
                          ...
                          {getNotePublishTime(note) ? (
                            <Text type="secondary" style={{ fontSize: 11, marginLeft: 6 }}>
                              {getNotePublishTime(note)}
                            </Text>
                          ) : (
                            <Text type="secondary" style={{ fontSize: 11, marginLeft: 6 }}>
                              {formatSavedTime(note.created_at)}
                            </Text>
                          )}
                        </div>
                        {(() => {
                          ...
                          return (
                            <div style={{ marginTop: 4, display: "flex", gap: 8, fontSize: 11, color: "rgba(255,255,255,.45)" }}>
                              {eng.likes > 0 && (
                                <span>
                                  <HeartOutlined /> {eng.likes}
                                </span>
                              )}
                              {eng.collects > 0 && (
                                <span>
                                  <StarOutlined /> {eng.collects}
                                </span>
                              )}
                              {eng.comments > 0 && (
                                <span>
                                  <MessageOutlined /> {eng.comments}
                                </span>
                              )}
                              {eng.shares > 0 && (
                                <span>
                                  <ShareAltOutlined /> {eng.shares}
                                </span>
                              )}
                            </div>
                          );
                  ...
                  {note.tags?.length ? (
                    <div style={{ marginTop: 6 }}>
                      {note.tags.map((t) => (
                        <Tag key={t.id} color="blue" style={{ fontSize: 11 }}>
                          {t.name}
                        </Tag>
                      ))}
                    </div>
          ...
          <Spin spinning={isDetailLoading}>
            ...
            {detailActionMessage && (
              <Alert
                ...
                closable
                onClose={() => setDetailActionMessage(null)}
              />
            )}
            ...
            <Descriptions column={1} size="small" style={{ marginBottom: 16 }}>
              <Descriptions.Item label="作者">
                {getAuthorProfileUrl(selectedNote) ? (
                  <Typography.Link href={getAuthorProfileUrl(selectedNote)} target="_blank" rel="noreferrer">
              ...
              <Descriptions.Item label="互动">
                赞 {getNoteEngagement(selectedNote).likes} · 藏 {getNoteEngagement(selectedNote).collects} · 评{" "}
                {getNoteEngagement(selectedNote).comments}
              </Descriptions.Item>
              ...
              {getNotePublishTime(selectedNote) && (
                <Descriptions.Item label="发布时间">{getNotePublishTime(selectedNote)}</Descriptions.Item>
              )}
              <Descriptions.Item label="作品链接">
                <Typography.Link
                  href={getNoteUrl(selectedNote)}
                  target="_blank"
                  rel="noreferrer"
                  style={{ fontSize: 12, wordBreak: "break-all" }}
                >
                  {getNoteUrl(selectedNote)}
                </Typography.Link>
              </Descriptions.Item>
            </Descriptions>
            ...
            <Space wrap style={{ marginBottom: 16 }}>
              <Button icon={<CopyOutlined />} onClick={copySelectedNote} size="small">
                复制内容
              </Button>
              <Button icon={<FileAddOutlined />} onClick={addToDrafts} loading={isCreatingDraft} size="small">
                加入草稿工坊
              </Button>
              <Button icon={<EditOutlined />} onClick={() => createDraft("rewrite")} loading={isCreatingDraft} size="small">
                AI 改写
              </Button>
              <Popconfirm title="确定删除？" onConfirm={() => void handleDeleteNote(selectedNote)}>
                  ...
                  <Space size={8} wrap>
                    {selectedAssets.map((a) =>
                      a.asset_type === "video" ? (
                        ...
                        >
                          <Button type="link" icon={<PlayCircleOutlined />} href={a.url} target="_blank" rel="noreferrer">
                            视频
                          </Button>
                        </div>
                      ) : (
            ...
            <div style={{ marginBottom: 16 }}>
              <Text strong>正文</Text>
              <Paragraph style={{ marginTop: 4, color: "rgba(255,255,255,.65)", whiteSpace: "pre-wrap" }}>
                {selectedNote.content || "暂无正文。"}
              </Paragraph>
            </div>

            <Button onClick={toggleComments} style={{ marginBottom: 8 }}>
              {isCommentsOpen ? "收起评论" : `查看评论 (${commentsTotal})`}
            </Button>
            {isCommentsOpen && (
              <Card size="small" style={{ background: "#1f1f1f" }}>
                  ...
                  <div key={c.comment_id} style={{ marginBottom: 10, paddingBottom: 8, borderBottom: "1px solid #303030" }}>
                    <Space>
                      ...
                      <Text type="secondary" style={{ fontSize: 11 }}>
                        {c.created_at_remote} · {c.like_count} likes
                      </Text>
                    </Space>
                ...
                {comments.length < commentsTotal && (
                  <Button size="small" onClick={() => void loadComments(commentsPage + 1)} loading={isCommentsLoading}>
                    加载更多
                  </Button>
                )}
              </Card>
            ...
            {selectedNote.raw_json && (
              <details style={{ marginTop: 16 }}>
                ...
                >
                  {JSON.stringify(selectedNote.raw_json, null, 2)}
                </pre>
              </details>
            )}
          </Spin>
