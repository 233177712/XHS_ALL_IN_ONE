...
import { Alert, Button, Card, Checkbox, Col, Empty, Form, Input, InputNumber, Row, Select, Space, Table, Tag, Typography } from "antd";
...
function spiderStyleNoteRow(item: XhsDataCrawlItem): string[] {
  ...
  const note_url = textValue(
    note.note_url,
    card.note_url,
    card.url,
    rawItem.note_url,
    rawItem.url,
    item.source.startsWith("http") ? item.source : "",
  );
  ...
  return [
    textValue(note.note_id, card.note_id, card.id, rawItem.id),
    note_url,
    noteTypeText(textValue(note.type, card.type, rawItem.model_type)),
    user_id,
    user_id ? `https://www.xiaohongshu.com/user/profile/${user_id}` : "",
    ...
    textValue(note.cover_url, video.cover_url),
    video_addr,
    listValue(note.image_urls?.length ? note.image_urls : note.cover_url ? [note.cover_url] : []),
    listValue(note.tags),
    upload_time,
    textValue(card.ip_location, rawItem.ip_location),
  ];
}
...
export function XhsCrawlerPage() {
  ...
  async function handleRun(e?: FormEvent) {
    ...
    if (mode !== "search" && parsedUrls.length === 0) {
      setError("请至少输入一个笔记链接。");
      return;
    }
      ...
      const summary = await crawlXhsDataStream(
        {
          ...
          fetch_comments: mode === "comments" ? false : fetchCommentsChecked,
          ...filters,
          geo: filters.geo.trim(),
        },
        (index, item) => {
        ...
        <Form layout="vertical" onFinish={() => void handleRun()}>
          <Row gutter={16}>
            ...
            <Col span={4}>
              <Form.Item label="Time Sleep">
                <InputNumber
                  min={0}
                  max={MAX_XHS_CRAWL_INTERVAL_SECONDS}
                  step={0.5}
                  value={timeSleep}
                  onChange={(v) => setTimeSleep(v ?? 120)}
                  style={{ width: "100%" }}
                />
              </Form.Item>
            </Col>
                ...
                <Form.Item label="爬取数量">
                  <InputNumber
                    ...
                    onChange={(v) => {
                      const n = v ?? 20;
                      setMaxNotes(n);
                      setPages(Math.max(1, Math.ceil(n / 20)));
                    }}
                    style={{ width: "100%" }}
                  />
                </Form.Item>
              ...
              <Col span={4}>
                <Form.Item label="距离">
                  <Select
                    ...
                    onChange={(v) => setFilters((c) => ({ ...c, pos_distance: v }))}
          ...
          <Space>
            <Button
              ...
              htmlType="submit"
              loading={isRunning}
              disabled={noPcAccount}
              icon={mode === "search" ? <SearchOutlined /> : <CloudDownloadOutlined />}
            >
              {isRunning ? "抓取中..." : "开始抓取"}
            </Button>
            <Button icon={<FileExcelOutlined />} onClick={() => items.length && exportRowsToExcel(items)} disabled={!items.length}>
              导出 Excel
            </Button>
          </Space>
        </Form>
