...
import { Alert, Button, Card, Col, Descriptions, Drawer, Empty, Input, Row, Select, Space, Spin, Tag, Typography } from "antd";
...
import {
  fetchAccounts,
  fetchSavedNoteIds,
  fetchXhsNoteComments,
  fetchXhsNoteDetail,
  saveXhsNotesToLibrary,
  searchXhsNotes,
} from "../../../lib/api";
...
function getPreviewNoteUrl(note: XhsSearchNote): string {
  return (
    note.note_url ||
    rawString(note, ["note_url", "url", "share_url"]) ||
    (note.note_id ? `https://www.xiaohongshu.com/explore/${note.note_id}` : "")
  );
}
function getCoverUrl(note: XhsSearchNote): string {
  return note.cover_url || note.image_urls?.[0] || rawString(note, ["cover_url", "image_url"]);
}
function getNoteImageUrls(note: XhsSearchNote): string[] {
  const urls = note.image_urls?.length ? note.image_urls : [getCoverUrl(note)];
  return urls.filter((u): u is string => Boolean(u));
}
function getNoteVideoUrl(note: XhsSearchNote): string {
  return note.video_url || note.video_addr || rawString(note, ["video_url", "video_addr"]);
}
...
export function XhsDiscoveryPage() {
  ...
  const pcAccountOptions = useMemo(
    () => pcAccounts.map((a) => ({ value: a.id, label: `${a.nickname || `PC ${a.id}`} · ${a.status}` })),
    [pcAccounts],
  );
  ...
  async function runSearch(nextPage: number, append: boolean) {
    ...
    if (!payload) return;
    setError(null);
    append ? setIsLoadingMore(true) : setIsSearching(true);
    try {
      ...
      setCommentPreviewByNoteId((c) => (append ? c : {}));
      setCommentPreviewErrors((c) => (append ? c : {}));
      setSearchedKeyword(payload.keyword);
    } catch (err: unknown) {
      const a = err as { response?: { status?: number; data?: { detail?: string } }; message?: string };
      setError(
        a?.response?.data?.detail
          ? `[${a.response.status}] ${a.response.data.detail}`
          : `搜索失败：${a?.message || "请检查网络和后端服务"}`,
      );
    } finally {
  ...
  async function ensureNoteDetail(note: XhsSearchNote): Promise<XhsSearchNote> {
    ...
    setSelectedNote((c) => (c?.note_id === note.note_id ? merged : c));
    return merged;
  }
  ...
  async function handleFetchUrlDetail() {
    ...
    try {
      ...
      setHasMore(false);
      setPage(1);
      setSearchedKeyword("URL 直查");
      setSavedNoteIds([]);
      setCommentPreviewByNoteId({});
      setCommentPreviewErrors({});
    } catch (err: unknown) {
      const a = err as { response?: { status?: number; data?: { detail?: string } }; message?: string };
      setError(
        a?.response?.data?.detail ? `[${a.response.status}] ${a.response.data.detail}` : `URL 直查失败：${a?.message || "请检查网络"}`,
      );
    } finally {
  ...
  async function openDetail(note: XhsSearchNote) {
    ...
    setDetailError(null);
    if (!selectedAccountId) {
      setDetailError("请先选择一个 PC 账号。");
      return;
    }
    const url = getPreviewNoteUrl(note);
    if (!url) {
      setDetailError("缺少可用于获取详情的笔记 URL。");
      return;
    }
    ...
    try {
      const detail = await fetchXhsNoteDetail({ account_id: selectedAccountId, url });
      setDetailMediaIndex(0);
      setSelectedNote({ ...note, ...detail, note_url: detail.note_url || url });
    } catch {
      setDetailError("详情加载失败，已保留搜索结果预览。");
    } finally {
  ...
  async function handlePreviewComments(note: XhsSearchNote) {
    ...
    const url = getPreviewNoteUrl(note);
    if (!url) {
      setCommentPreviewErrors((c) => ({ ...c, [note.note_id]: "缺少笔记 URL。" }));
      return;
    }
    if (commentPreviewByNoteId[note.note_id]) {
      setCommentPreviewByNoteId((c) => {
        const n = { ...c };
        delete n[note.note_id];
        return n;
      });
      return;
    }
    setLoadingCommentNoteIds((c) => [...c, note.note_id]);
    setCommentPreviewErrors((c) => ({ ...c, [note.note_id]: "" }));
    try {
      ...
      setCommentPreviewErrors((c) => ({ ...c, [note.note_id]: "评论加载失败。" }));
    } finally {
      setLoadingCommentNoteIds((c) => c.filter((id) => id !== note.note_id));
    }
  }
    ...
    <div>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
          ...
          <Text type="secondary">按关键词或链接查找笔记，查看详情、评论和原文，保存到内容库</Text>
        </Col>
        <Col>
          <Button icon={<ReloadOutlined />} onClick={loadAccounts} loading={isLoadingAccounts}>
            刷新账号
          </Button>
        </Col>
      </Row>
        ...
        >
          <Row gutter={[12, 12]}>
            ...
            <Col span={6}>
              ...
              <Input.Search
                ...
                loading={isSearching}
                onSearch={() => void runSearch(1, false)}
                enterButton
              />
            </Col>
            <Col span={3}>
              ...
              <Select
                ...
                onChange={(v) => setFilters((c) => ({ ...c, sort_type_choice: v }))}
                style={{ width: "100%" }}
                options={sortOptions}
              />
            </Col>
            <Col span={3}>
              ...
              <Select
                ...
                onChange={(v) => setFilters((c) => ({ ...c, note_type: v }))}
                style={{ width: "100%" }}
            ...
            <Col span={3}>
              ...
              <Select
                ...
                onChange={(v) => setFilters((c) => ({ ...c, note_time: v }))}
                style={{ width: "100%" }}
                options={noteTimeOptions}
              />
            </Col>
            <Col span={3}>
              <div style={{ marginBottom: 4 }}>
              ...
              <Select
                value={filters.note_range}
                onChange={(v) => setFilters((c) => ({ ...c, note_range: v }))}
                style={{ width: "100%" }}
          ...
          <Row gutter={12} style={{ marginTop: 12 }} align="bottom">
            <Col span={6}>
              ...
              <Input value={noteUrl} onChange={(e) => setNoteUrl(e.target.value)} placeholder="https://www.xiaohongshu.com/explore/..." />
            </Col>
            <Col>
              <Button icon={<SearchOutlined />} loading={isFetchingUrl} disabled={noPcAccount} onClick={handleFetchUrlDetail}>
                URL 直查
              </Button>
            </Col>
          </Row>
        ...
        title={
          <Space>
            <Title level={5} style={{ margin: 0 }}>
              {searchedKeyword ? `"${searchedKeyword}" 的搜索结果` : "搜索结果"}
            </Title>
            <Tag>{notes.length} 篇</Tag>
          </Space>
        }
      >
        {notes.length === 0 ? (
          <Empty
            ...
            description={searchedKeyword ? "这次搜索没有返回笔记。" : "输入关键词后，搜索结果会以笔记卡片显示在这里。"}
          />
        ) : (
          <>
            <Row gutter={[16, 16]}>
                  ...
                  <Col xs={12} sm={8} md={6} lg={4} xl={4} key={`${note.note_id}-${note.title}`}>
                    <Card
                      ...
                      cover={
                        <div style={{ position: "relative", background: "#262626" }}>
                          ...
                          <Tag
                            color={kind === "视频" ? "purple" : "blue"}
                            style={{ position: "absolute", top: 8, left: 8 }}
                            icon={kind === "视频" ? <PlayCircleOutlined /> : <PictureOutlined />}
                          >
                            {kind}
                          </Tag>
                        </div>
                    ...
                    >
                      ...
                      <Space size={12} style={{ marginTop: 8, fontSize: 12, color: "rgba(255,255,255,.45)" }}>
                        <span>
                          <HeartOutlined /> {formatMetric(note.likes)}
                        </span>
                        <span>
                          <StarOutlined /> {formatMetric(note.collects)}
                        </span>
                        <span>
                          <CommentOutlined /> {formatMetric(note.comments)}
                        </span>
                      </Space>
                      <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }} onClick={stopCardClick}>
                        <Button
                          size="small"
                          type={savedNoteIds.includes(note.note_id) ? "default" : "primary"}
                          ghost={!savedNoteIds.includes(note.note_id)}
                          icon={savedNoteIds.includes(note.note_id) ? <CheckOutlined /> : <DatabaseOutlined />}
                          loading={savingNoteIds.includes(note.note_id)}
                          disabled={savedNoteIds.includes(note.note_id)}
                        ...
                        <Button
                          ...
                          loading={loadingCommentNoteIds.includes(note.note_id)}
                          onClick={() => void handlePreviewComments(note)}
                        >
                      ...
                      {commentPreviewErrors[note.note_id] && (
                        <Alert message={commentPreviewErrors[note.note_id]} type="error" style={{ marginTop: 8, fontSize: 12 }} showIcon />
                      )}
                      {commentPreviewByNoteId[note.note_id] && (
                        <div style={{ marginTop: 8, borderTop: "1px solid #303030", paddingTop: 8 }} onClick={stopCardClick}>
                          {commentPreviewByNoteId[note.note_id].length === 0 ? (
                            <Text type="secondary" style={{ fontSize: 12 }}>
                              暂无评论
                            </Text>
                          ) : null}
            ...
            <div style={{ textAlign: "center", marginTop: 24 }}>
              <Button onClick={handleLoadMore} disabled={!hasMore || isLoadingMore} loading={isLoadingMore}>
                {hasMore ? "加载更多" : "没有更多了"}
              </Button>
            </div>
          </>
        ...
        {selectedNote && (
          <div>
                ...
                <div style={{ position: "relative", background: "#262626", borderRadius: 8, overflow: "hidden" }}>
                  {selImgUrls.length ? (
                    ...
                    <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <PlayCircleOutlined style={{ fontSize: 40, color: "rgba(255,255,255,.3)" }} />
                    </div>
                  )}
                  <Tag color="purple" style={{ position: "absolute", top: 8, left: 8 }}>
                    <PlayCircleOutlined /> 视频封面
                  </Tag>
                </div>
                <Button
                  type="primary"
                ...
                <div style={{ position: "relative", background: "#262626", borderRadius: 8, overflow: "hidden", textAlign: "center" }}>
                  ...
                  {selImgUrls.length > 1 && (
                    <>
                      ...
                      />
                      <Tag style={{ position: "absolute", bottom: 8, right: 8 }}>
                        {selMediaIdx + 1}/{selImgUrls.length}
                      </Tag>
                    </>
                  )}
                </div>
                    ...
                    {selImgUrls.map((url, i) => (
                      <div
                        ...
                        style={{
                          ...
                          overflow: "hidden",
                          cursor: "pointer",
                          border: i === selMediaIdx ? "2px solid #1668dc" : "2px solid transparent",
                          flexShrink: 0,
                        }}
                      >
            ...
            <Descriptions column={1} size="small" style={{ marginBottom: 16 }}>
              <Descriptions.Item label="作者">
                {selectedNote.author_id ? (
                  <Typography.Link
                    href={`https://www.xiaohongshu.com/user/profile/${selectedNote.author_id}`}
                    target="_blank"
                    rel="noreferrer"
                  >
              ...
              <Descriptions.Item label="互动">
                赞 {formatMetric(selectedNote.likes)} · 藏 {formatMetric(selectedNote.collects)} · 评 {formatMetric(selectedNote.comments)}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="作品链接">
                <Typography.Link
                  href={getPreviewNoteUrl(selectedNote)}
                  target="_blank"
                  rel="noreferrer"
            ...
            <Space wrap style={{ marginBottom: 16 }}>
              ...
              <Button icon={<CommentOutlined />} onClick={() => void handlePreviewComments(selectedNote)}>
                {commentPreviewByNoteId[selectedNote.note_id] ? "收起评论" : "查看评论"}
              </Button>
              {getPreviewNoteUrl(selectedNote) && (
                <Button type="primary" icon={<LinkOutlined />} href={getPreviewNoteUrl(selectedNote)} target="_blank" rel="noreferrer">
                  打开原文
                </Button>
              )}
            </Space>
            ...
            {commentPreviewErrors[selectedNote.note_id] && (
              <Alert message={commentPreviewErrors[selectedNote.note_id]} type="error" showIcon style={{ marginBottom: 12 }} />
            )}
